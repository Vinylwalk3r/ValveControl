...
void Open()
{                                 // The sequnce of commands to run when wanting to open the valve
...
void Close()
{                                 // Call this code to close the valve
...
{                               // This function will be called if the Pressure sensor gives an alert
...
void Sensor_Failure()
{ // This function is called on if the senor fails
  digitalWrite(Redlight, HIGH);
  delay(2000);
  digitalWrite(Redlight, LOW);
  delay(2000);
 ...
 */
  ...
  if (MoistureSensor == HIGH)
  { // Checks the sensors feedback, this would be the signal from the 433mhz reciver
    for (RuntimeCounter = 0; RuntimeCounter < 1; RuntimeCounter++)
    {                                 // This limits the time this "if" can run to 1 time. If this would loop, then the valve would open and close repeatedly
...
if (PhoneInHouse == 0) {      // Activates if the phone leaves the house.
  ...
  if (Pressure < 682)
  { //This checks the integer "Pressure", and activates if the value is 682 or lower
    for (RuntimeCounter = 0; RuntimeCounter < 1; RuntimeCounter++)
    {                                 // This limits the time this "if" can run to 1 time. If this would loop, then the valve would open and close repeatedly
      digitalWrite(Watervalve, HIGH); // This closes the valve
      delay(1000);
  ...
  if (digitalRead(SCIn) == HIGH)
  { // Monitors the status of the sensor, activates if a fail message is recived
    for (RuntimeCounter = 0; RuntimeCounter < 1; RuntimeCounter++)
    {                                 // This limits the time this "if" can run to 1 time. If this would loop, then the valve would open and close repeatedly
      digitalWrite(Watervalve, HIGH); // This closes the valve
      delay(1000);
